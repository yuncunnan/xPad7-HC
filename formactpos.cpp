...
#include "ui_formactpos.h"
#include "servoalias.h"
#include "xstringresource.h"
#include "xmessagebox.h"

FormActPos::FormActPos(QWidget *parent) : QWidget(parent), ui(new Ui::FormActPos)
{
	// 初始化界面
	ui->setupUi(this);
	retranslateUi();
...
#if FACTORY == FA_RUIXIANG
    ui->checkOffset->setVisible(true);
#else
    ui->checkOffset->setVisible(false);
#endif
}

FormActPos::~FormActPos()
{
	delete ui;
	delete pAxesBtn;
}

void FormActPos::changeEvent(QEvent *e)
{
...
void FormActPos::retranslateUi(void)
{
...
void FormActPos::SetType(quint8 type)
{
...
quint32 FormActPos::GetSpeed()
{
	return ui->BoxSpeed->value();
}

void FormActPos::SetSpeed(int speed)
{
	ui->BoxSpeed->setValue((double)speed);
}

int FormActPos::GetAxes()
{
	return pAxesBtn->checkedId();
}

void FormActPos::SetAxes(int axes)
{
...
void FormActPos::AxesClick(int axes)
{
    UpdateWidgetState();
}
...
void FormActPos::SetX(int pos, int type)
{
	m_x_type = type;
    m_x_pos = pos;
	switch(type)
...
void FormActPos::SetY(int pos, int type)
{
   m_y_type = type;
   m_y_pos = pos;
    switch(type)
...
void FormActPos::SetZ(int pos, int type)
{
    m_z_type = type;
    m_z_pos = pos;
    switch(type)
...
void FormActPos::SetA(int pos, int type)
{
	m_a_type = type;
    m_a_pos = pos;
    switch(type)
...
void FormActPos::SetB(int pos, int type)
{
	m_b_type = type;
    m_b_pos = pos;
    switch(type)
...
void FormActPos::SetC(int pos, int type)
{
	m_c_type = type;
    m_c_pos = pos;
    switch(type)
...
// 输入伺服速度槽
void FormActPos::inputSpeed(void)
{
...
// 输入正臂引拔位置信号槽
void FormActPos::inputPaHor(void)
{
...
// 同步正臂引拔位置信号槽
void FormActPos::syncPaHor(void)
{
    m_x_pos = CMBProtocol::GetPaHorPos();
    ui->BoxPaHor->setText(QString::number((double)m_x_pos / 100.00) + "mm");
}
// 输入正臂上下位置信号槽
void FormActPos::inputPaVer(void)
{
    int type = POS_TYPE_POINT;
	DialogNumberPad numberPad;
	double value;
...
// 同步正臂上下位置信号槽
void FormActPos::syncPaVer(void)
{
    m_y_pos = CMBProtocol::GetPaVerPos();
    ui->BoxPaVer->setText(QString::number((double)m_y_pos / 100.00) + "mm");
}
// 输入横行位置信号槽
void FormActPos::inputTravel(void)
{
    int type = POS_TYPE_POINT;
	DialogNumberPad numberPad;
	double value;
...
// 同步横行位置信号槽
void FormActPos::syncTravel(void)
{
    m_z_pos = CMBProtocol::GetTravelPos();
    ui->BoxTrv->setText(QString::number((double)m_z_pos / 100.00) + "mm");
}
// 输入副臂引拔位置信号槽
void FormActPos::inputRaHor(void)
{
    int type = POS_TYPE_POINT;
	DialogNumberPad numberPad;
	double value;
...
// 同步副臂引拔位置信号槽
void FormActPos::syncRaHor(void)
{
    m_b_pos = CMBProtocol::GetRaHorPos();
    ui->BoxRaHor->setText(QString::number((double)m_b_pos / 100.00) + "mm");
}
// 输入副臂上下位置信号槽
void FormActPos::inputRaVer(void)
{
    int type = POS_TYPE_POINT;
	DialogNumberPad numberPad;
	double value;
...
// 同步副臂引拔位置信号槽
void FormActPos::syncRaVer(void)
{
    m_a_pos = CMBProtocol::GetRaVerPos();
    ui->BoxRaVer->setText(QString::number((double)m_a_pos / 100.00) + "mm");
}
// 输入扩展位置信号槽
void FormActPos::inputOther(void)
{
    int type = POS_TYPE_POINT;
	DialogNumberPad numberPad;
	double value;
...
// 同步扩展位置信号槽
void FormActPos::syncOther(void)
{
    m_c_pos = CMBProtocol::GetExtendPos();
    ui->BoxOther->setText(QString::number((double)m_c_pos / 100.00) + "mm");
}
void FormActPos::UpdateWidgetState()
{
    if (CMBProtocol::GetTemplateUse())
    {
//        if (visible)
...
// 输入延时参数信号槽
void FormActPos::inputDelay(void)
{
    DialogNumberPad numberPad;
    double value;
...
void FormActPos::SetDelay(int delay)
{
    ui->BoxDelay->setValue((double)delay / (1000.0/TIME_BASE));
}

int FormActPos::GetDelay()
{
