...
void xStringResource::initAlarmSolutionRes(void)
{
	...
	if ((CMBProtocol::GetSysType() == ROBOTARM_TYPE_SIXSV_PAD_E4) || (CMBProtocol::GetSysType() == ROBOTARM_TYPE_SIXSV_PAD_F4))
    {
        ...
        << tr("[258]")
        << tr("[259]")
        << tr("[260] ") + GetServoName(AXIS_IDX_PHOR) + tr("轴跟随误差过大")
        << tr("[261] ") + GetServoName(AXIS_IDX_PVER) + tr("轴跟随误差过大")
        << tr("[262] ") + GetServoName(AXIS_IDX_TRV)  + tr("轴跟随误差过大")
        << tr("[263] ") + GetServoName(AXIS_IDX_RVER) + tr("轴跟随误差过大")
        << tr("[264] ") + GetServoName(AXIS_IDX_RHOR) + tr("轴跟随误差过大")
        << tr("[265] ") + GetServoName(AXIS_IDX_EXT)  + tr("轴跟随误差过大")
        << tr("[266]")
        << tr("[267]")
        << tr("[268]")
        << tr("[269]")
...
void xStringResource::initAlarmRes(void)
{
    ...
    if ((CMBProtocol::GetSysType() == ROBOTARM_TYPE_SIXSV_PAD_E4) || (CMBProtocol::GetSysType() == ROBOTARM_TYPE_SIXSV_PAD_F4))
    {
       ...
       << tr("[094] %1轴伺服报警，请检查伺服驱动器。").arg(GetServoName(AXIS_IDX_EXT))
        ...
        << tr("[139] 气动副臂上行但电眼不亮。")
        << tr("[140] 气动副臂上行但下行电眼亮。")
#if ((FACTORY == FA_XIONGCHUANG) || (FACTORY == FA_VICTOR) || (FACTORY == FA_KEHUI) || (FACTORY == FA_WEILIHE) || (FACTORY == FA_SIGMA) ||  (FACTORY == FA_SINROBOT_CN) || (FACTORY == FA_BEISHITE_CN) || (FACTORY == FA_MIDEA))
    ...
    else
    {
        ...
        << tr("[025] 系统在错误状态，不能改变系统的工作状态。")
        << tr("[026] 系统正在运行，不能改变系统的工作状态。")
#if ((FACTORY == FA_AIERFA) || (FACTORY == FA_GH) || (FACTORY == FA_SINROBOT_JC) || (FACTORY == FA_KINGROBOT) || (FACTORY == FA_BEISHITE) ||(FACTORY == FA_XIONGCHUANG) || (FACTORY == FA_VICTOR) || (FACTORY == FA_KEHUI) || (FACTORY == FA_WEILIHE) || (FACTORY == FA_SIGMA) ||  (FACTORY == FA_SINROBOT_CN) || (FACTORY == FA_BEISHITE_CN) || (FACTORY == FA_MIDEA))
...
void xStringResource::initDvsERRRes()
{
    ...
    << tr("[030] 驱动器：编码器Z信号错误")
    << tr("[031] 驱动器：编码器UVW信号错误")
    << tr("[032] 驱动器：UVW信号存在全高或全低电平")
    << tr("[033] 驱动器：省线式编码器信号错误")
    << tr("[034] 驱动器：编码器信号错误")
    << tr("[035] 驱动器：")
    << tr("[036] 驱动器：编码器上电全高电平时间过长")
    << tr("[037] 驱动器：")
