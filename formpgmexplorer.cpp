...
FormPgmExplorer::FormPgmExplorer(CMBProtocol *modbus, QWidget *parent) : QWidget(parent), ui(new Ui::FormPgmExplorer)
{
	...
	// 设置表头字体
	ui->pgmView->horizontalHeader()->setFont(QFont("Microsoft YaHei", 18));
	ui->pgmView->verticalHeader()->setFont(QFont("Microsoft YaHei", 18));
	ui->temView->horizontalHeader()->setFont(QFont("Microsoft YaHei", 18));
	ui->temView->verticalHeader()->setFont(QFont("Microsoft YaHei", 18));
	ui->usbView->horizontalHeader()->setFont(QFont("Microsoft YaHei", 18));
	ui->usbView->verticalHeader()->setFont(QFont("Microsoft YaHei", 18));
	// 设置表头尺寸模式
	...
	// 设置列宽度
    ui->pgmView->setColumnWidth(1, 125);
    ui->temView->setColumnWidth(1, 125);
    ui->usbView->setColumnWidth(1, 125);
    ui->pgmView->setColumnWidth(3, 225);
    ui->temView->setColumnWidth(3, 225);
    ui->usbView->setColumnWidth(3, 225);
#if IS_XEDITOR
...
{
//    if (btnloadisclick == false)
//    {
//        if (lastname != "")
//        {
//            if ((xMessageBox::DoQuestion(tr("系统提醒"),tr("是否切换到新创建的程序？"))) == XMSG_RET_YES)
//                pModbus->SetCurPgmName(lastname);
//        }
//    }
    // 取消信号槽连接
	ui->usbView->setRootIndex(uDirModel->setRootPath("/root/"));
...
void FormPgmExplorer::newPgmFile(void)
{
    ...
    newPathName = newPathName + name + (PGM_FILE_EXT);
//	newPathName.append(PGM_FILE_EXT);
    ...
    if(xml.NewProgram())
    {
        if (xml.SaveProgram(newPathName))
		{
	...
	ui->pgmView->setRootIndex(pDirModel->setRootPath("/root/"));
	ui->pgmView->setRootIndex(pDirModel->setRootPath(PROGRAM_PATH));
}
// **********************************
// 重命名槽
