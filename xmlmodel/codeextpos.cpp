...
CodeExtPos::CodeExtPos(QWidget *form) : XmlModelItem(QIcon(), form)
{
    m_code = CODE_EXTPOS;
    m_form = (FormActExtPos*)form;
	m_axes = 1;
	m_speed = 50;
    ...
    m_advdec = false;      //提前变速
    m_advspeed = 10;        //变速速度
    m_advfinish = false;   //提前结束
    m_advdis = 0;   //提前距离
    m_enddis = 0;           //变速距离
    m_accspeed = 100;        //加速度
    m_jerkspeed = 100;        //jerk加加速

    UpdateBaseMember();
}
bool CodeExtPos::XmlReadPara(QXmlStreamReader &reader)
{
        ...
        (name==XML_PNAME_PARA15))
    {
        ...
        if (name==XML_PNAME_PARA10)
        {
            m_advspeed = val.toInt();
        }
        else
        if (name==XML_PNAME_PARA11)
        {
            m_advfinish = val.toInt();
        }
        else
        if (name==XML_PNAME_PARA12)
        {
            m_advdis = val.toInt();
        }
        else
        if (name==XML_PNAME_PARA13)
        {
            m_enddis = val.toInt();
        }
        else
        if (name==XML_PNAME_PARA14)
        {
            m_accspeed = val.toInt();
        }
        else
        if (name==XML_PNAME_PARA15)
        {
            m_jerkspeed = val.toInt();
        }
        UpdateBaseMember();
...
bool CodeExtPos::XmlRead(QXmlStreamReader &reader)
{
...
bool CodeExtPos::XmlWrite(QXmlStreamWriter &writer)
{
	writer.writeStartElement(XML_ELEMENT_CODE);
    writer.writeAttribute(XML_ATTR_NAME, XML_CNAME_EXTPOS);
    ...
    writer.writeAttribute(XML_ATTR_FLAG, QString("%1").arg(m_flag));
        ...
        writer.writeAttribute(XML_ATTR_NAME, XML_PNAME_PARA9);
            writer.writeCharacters(QString("%1").arg(m_advdec));
        writer.writeEndElement();

        writer.writeStartElement(XML_ELEMENT_PARA);
        writer.writeAttribute(XML_ATTR_NAME, XML_PNAME_PARA10);
            writer.writeCharacters(QString("%1").arg(m_advspeed));
        writer.writeEndElement();

        writer.writeStartElement(XML_ELEMENT_PARA);
        writer.writeAttribute(XML_ATTR_NAME, XML_PNAME_PARA11);
            writer.writeCharacters(QString("%1").arg(m_advfinish));
        writer.writeEndElement();

        writer.writeStartElement(XML_ELEMENT_PARA);
        writer.writeAttribute(XML_ATTR_NAME, XML_PNAME_PARA12);
            writer.writeCharacters(QString("%1").arg(m_advdis));
        writer.writeEndElement();

        writer.writeStartElement(XML_ELEMENT_PARA);
        writer.writeAttribute(XML_ATTR_NAME, XML_PNAME_PARA13);
            writer.writeCharacters(QString("%1").arg(m_enddis));
        writer.writeEndElement();

        writer.writeStartElement(XML_ELEMENT_PARA);
        writer.writeAttribute(XML_ATTR_NAME, XML_PNAME_PARA14);
            writer.writeCharacters(QString("%1").arg(m_accspeed));
        writer.writeEndElement();

        writer.writeStartElement(XML_ELEMENT_PARA);
        writer.writeAttribute(XML_ATTR_NAME, XML_PNAME_PARA15);
            writer.writeCharacters(QString("%1").arg(m_jerkspeed));
        writer.writeEndElement();

        writer.writeEndElement();
	return true;
}
bool CodeExtPos::Update(bool bSaveAndValidate)
{
	if (m_form)
	{
		if (bSaveAndValidate)
		{
            ...
            if (m_advdec != m_form->GetAdvDec())
            {
                m_advdec = m_form->GetAdvDec();
                change=true;
            }
            if (m_accspeed != m_form->GetAccSpeed())
            {
                m_accspeed = m_form->GetAccSpeed();
                change=true;
            }
            if (m_jerkspeed != m_form->GetJerkSpeed())
            {
                m_jerkspeed = m_form->GetJerkSpeed();
                change=true;
            }
            if (m_advspeed != m_form->GetAdvSpeed())
            {
                m_advspeed = m_form->GetAdvSpeed();
                change=true;
            }
            if (m_advfinish != m_form->GetAdvFinish())
            {
                m_advfinish = m_form->GetAdvFinish();
                change=true;
            }
            if (m_advdis != m_form->GetAdvDis())
            {
                m_advdis = m_form->GetAdvDis();
                change=true;
            }
            if (m_enddis != m_form->GetEndDis())
            {
                m_enddis = m_form->GetEndDis();
                change=true;
            }
		...
		else
		{
            ...
            m_form->SetAdvDec(m_advdec);
            m_form->SetAdvSpeed(m_advspeed);
            m_form->SetAccSpeed(m_accspeed);
            m_form->SetJerkSpeed(m_jerkspeed);
            m_form->SetAdvFinish(m_advfinish);
            m_form->SetAdvDis(m_advdis);
            m_form->SetEndDis(m_enddis);
...
bool CodeExtPos::GenerateCode(quint16* code, quint32 *para, quint8 other)
{
    ...
    quint32 advSpeed;
    advSpeed = qRound((double)(m_advspeed*CMBProtocol::GetSrvSpeed())/ 100.0);
    advSpeed = (advSpeed == 0 ? 1 : advSpeed);

    code[0] = CODE_EXTPOS | (((quint16)m_flag) << 8);
	para[0] = (m_axes & 0x3F);
	para[0] <<= 1;
	para[0] |= (m_type & 1);
	para[0] <<= 7;
	para[0] |= (relSpd & 0x7F);
    if (m_advdec) para[0] |= ((quint32)1 << 14);
    if (m_advfinish) para[0] |= ((quint32)1 << 15);
    ...
    para[2] = ((advSpeed&0x7F)<<24)|(m_advdis&0x00FFFFFF);
...
void CodeExtPos::UpdateBaseMember()
{
	QString type;
